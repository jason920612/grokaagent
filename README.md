...
- **編輯區分頁**：「主對話」加上每個開啟的子代理。子代理分頁是**唯讀**的完整工作紀錄，包含它收到的指示、思考、每次工具呼叫與差異；子代理只由主代理指揮。
...
快捷鍵（TUI 與網頁相同）：`Ctrl+B` 側欄、`Ctrl+J` 面板、`Alt+1`–`Alt+5` 切換側欄檢視、`Alt+0` 回主對話、`Alt+←/→` 切換分頁、`Ctrl+W` 關閉子代理分頁（TUI）。TUI 另有 `F3`／`F4` 對應側欄／面板；在子代理分頁直接打字會切回主對話的輸入框。`Shift+Enter` 換行，`Ctrl+Enter` 調整目前工作。
