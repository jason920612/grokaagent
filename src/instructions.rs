...
- read_file: UTF-8 text relative to the workspace. Paths cannot escape the workspace. Every line is prefixed with its 1-based number as `N|text` (header gives path and total lines). Use those numbers with write_file line/end_line. Optional start_line/end_line to read a slice. Optional pattern (regex/keyword) returns only matching lines, each with a `line` field and a `numbered` dump. Optional context/max_matches with pattern.
- write_file: UTF-8 in the workspace. The tool result is JSON whose `diff` field is a git unified diff (`--- a/` `+++ b/` `@@`) of the file before vs after; also `kind`, `plus`, `minus`. Read that diff to confirm the edit. Modes: (1) path+contents overwrites the whole file; (2) path+line+contents replaces that 1-based line (end_line for an inclusive range) with contents, which may be multiple lines; (3) path+pattern+contents regex-replaces every match (`$1` captures). Do not set line and pattern together. Line/pattern edits require the file to exist. line = last+1 appends.
